...
kosc* Stol::losuj_i_usun() {
	...
	std::uniform_int_distribution<> distrib(0, dlugosc - 1);
...
void Stol::wypisz_kosci_na_stole() {
	...
	std::cout << "Kosci na stole:" << std::endl << std::endl;
	int counter = 0;
	const int MAX_KOSCI = 28;
	while (glowa != nullptr&&counter<MAX_KOSCI) {
		std::cout <<"[" << glowa->oczko1 << "|" << glowa->oczko2 <<"] ";
		glowa = glowa->next;
		++counter;
	}
...
void Stol::doloz_kosc(kosc* nowa) {
	...
	if (nowa->oczko1 == wartosc_na_glowie) {
		std::swap(nowa->oczko1, nowa->oczko2);
	}
	...
	std::cout << "Blad: Kosc [" << nowa->oczko1 << "|" << nowa->oczko2 << "] nie pasuje do glowy ["
